import { View, Text, StyleSheet, ScrollView, Modal, Pressable, TextInput, KeyboardAvoidingView, Platform } from 'react-native';
import { useState } from 'react';
import { colors, typography } from '@/theme';
import { ScheduleItem } from '@/components/ScheduleItem';
import { WeekSelector } from '@/components/WeekSelector';
import { DisciplineDetails, DisciplineDetailsModal } from '@/components/DisciplineDetailsModal';
import { WeeklyScheduleGrid, GridClass } from '@/components/WeeklyScheduleGrid';
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { BookOpen, Clock3, X } from 'lucide-react-native';

const dayLabels = ['Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'];
...
export default function Horarios() {
    ...
    const [selectedClass, setSelectedClass] = useState<DisciplineDetails | null>(null);
    ...
    const periodString = monday.getMonth() === saturday.getMonth()
    ...
    const scheduleItems = [
        {
            ...
            startTime: '08:00',
            endTime: '10:00',
            details: { id: 'linear-algebra', name: 'Álgebra Linear', code: 'CC2666', professor: 'Prof. Weber', local: 'Lab 3 - Bloco D', frequency: 88 },
        },
        {
            ...
            startTime: '10:00',
            endTime: '12:00',
            details: { id: 'object-oriented-programming', name: 'Programação Orientada a Objetos', code: 'CC2666', professor: 'Prof. Freire', local: 'Lab 3 - Bloco D', frequency: 92 },
        },
        {
            ...
            startTime: '14:00',
            endTime: '16:00',
            details: { id: 'calculo-3', name: 'Cálculo 3', code: 'CC2666', professor: 'Prof. Waldemar', local: 'Lab 3 - Bloco D', frequency: 78 },
        },
    ];
    ...
    const initialGridClasses: GridClass[] = [
        // segunda
        { id: 'g1', discipline: 'Álgebra Linear', startTime: '08:00', endTime: '10:00', dayIndex: 0 },
        { id: 'g2', discipline: 'Prog. OO', startTime: '10:00', endTime: '12:00', dayIndex: 0 },
        { id: 'g3', discipline: 'Cálculo 3', startTime: '14:00', endTime: '16:00', dayIndex: 0 },
        // terça
        { id: 'g4', discipline: 'Física 2', startTime: '08:00', endTime: '10:00', dayIndex: 1, color: '#7C63D9' },
        { id: 'g5', discipline: 'Estrutura de Dados', startTime: '14:00', endTime: '16:00', dayIndex: 1, color: '#7C63D9' },
        // quarta
        { id: 'g6', discipline: 'Álgebra Linear', startTime: '10:00', endTime: '12:00', dayIndex: 2 },
        { id: 'g7', discipline: 'Banco de Dados', startTime: '16:00', endTime: '18:00', dayIndex: 2, color: '#2E9E6B' },
        // quinta
        { id: 'g8', discipline: 'Cálculo 3', startTime: '08:00', endTime: '10:00', dayIndex: 3 },
        { id: 'g9', discipline: 'Física 2', startTime: '14:00', endTime: '16:00', dayIndex: 3, color: '#7C63D9' },
        // sexta
        { id: 'g10', discipline: 'Prog. OO', startTime: '08:00', endTime: '10:00', dayIndex: 4 },
        { id: 'g11', discipline: 'Banco de Dados', startTime: '10:00', endTime: '12:00', dayIndex: 4, color: '#2E9E6B' },
    ];

    const [gridClasses, setGridClasses] = useState<GridClass[]>(initialGridClasses);

    // indice numerico do dia
    const selectedDayIndex = weekDays.findIndex((day) => day.date === selectedDay);

    //filtra para pegar apenas as aulas do dia
    const classesOfTheDay = gridClasses.filter(cls => cls.dayIndex === selectedDayIndex);
    ...
    function saveClassChanges() {
        ...
        setGridClasses((currentClasses) => currentClasses.map((cls) =>
    ...
    const hasInvalidTime = hasTimeInput &&
        (!isValidTime(startTime.trim()) || !isValidTime(endTime.trim()) || startTime.trim() >= endTime.trim());
    ...
    return (
        ...
        <ScrollView style={styles.container} contentContainerStyle={{paddingTop: insets.top, paddingBottom: insets.bottom}}>
            ...
            <View>
                { !selectedDay ? (
                    <View style={{backgroundColor: `${colors.surface}66`, borderRadius: 12, padding: 50, marginBottom: 8, borderColor: colors.border, borderWidth: 1.5, borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center'}}>
                        <Text style={{fontFamily: typography.fontFamily.regular, fontSize: typography.fontSize.xs, color: colors.textTertiary}}>Selecione um dia para ver os horários</Text>
                    </View>
                    ...
                    <>
                        <View style={styles.legend}>
                            ...
                            <View style={styles.legendItem}>
                                <View style={[styles.legendColor, { backgroundColor: colors.primaryBlue }]} />
                                <Text style={styles.legendText}>Agora</Text>
                            </View>
                            <View style={styles.legendItem}>
                                <View style={[styles.legendColor, { backgroundColor: colors.attention }]} />
                        ...
                        {classesOfTheDay.length > 0 ? (
                            classesOfTheDay.map((item) => (
                                <ScheduleItem
                                    ...
                                    onPress={() => {
                                        setSelectedClass(scheduleItems.find((s) => s.discipline === item.discipline)?.details || null);
                                    }}
                                />
                            ))
                        ) : (
                            <View style={{ paddingVertical: 30, alignItems: 'center', backgroundColor: `${colors.surface}66`, borderRadius: 12, marginBottom: 8, borderColor: colors.border, borderWidth: 1.5, borderStyle: 'dashed' }}>
            ...
            <WeeklyScheduleGrid
                ...
                onClassPress={(cls) => {
                    ...
                    // se não estiver editando, abre o modal com os detalhes da disciplina
                        ...
                        const match = scheduleItems.find((s) => s.discipline === cls.discipline);
        ...
        >
            ...
            >
                <Pressable style={StyleSheet.absoluteFill} onPress={closeClassEditor} />
                <View style={[styles.sheet, { paddingBottom: insets.bottom + 24 }]}>
                    ...
                    <View style={styles.sheetHeader}>
                        <View style={styles.sheetHeaderText}>
                            ...
                            <Text style={styles.sheetSubtitle}>
                                {editingClass ? `${dayLabels[editingClass.dayIndex]} • ${editingClass.startTime} - ${editingClass.endTime}` : ''}
                            </Text>
                        </View>
                        <Pressable
                    ...
                    {hasInvalidTime && (
                        <Text style={styles.errorText}>
                            Use horários no formato 08:00 e certifique-se de que o fim seja depois do início.
                        </Text>
                    )}
